...
from app.security import generate_key, generate_api_tokens, log_event
...
async def reset_user_hwid(user_id: int):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET hwid_hash = '' WHERE id = ?", (user_id,))
    conn.commit()
    conn.close()
...
async def toggle_seller_key(key_id: int):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT is_active FROM seller_api_keys WHERE id = ?", (key_id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
    ...
    cursor.execute("UPDATE seller_api_keys SET is_active = ? WHERE id = ?", (new_state, key_id))
    conn.commit()
    conn.close()
...
async def sandbox_execute(req: SandboxExecuteRequest):
    ...
    # Internal execution mapping to avoid network round-trip overhead or firewall issues
    endpoint = req.endpoint.strip()
    method = req.method.upper()
    body = req.body or {}

    try:
        # We can perform local dispatch
        import httpx
        # We simulate high performance local invocation
    except ImportError:
        pass

    # We do a direct internal call or requests call to 127.0.0.1:8000
    try:
        url = f"http://127.0.0.1:8000{endpoint}" if endpoint.startswith("/") else f"http://127.0.0.1:8000/{endpoint}"
        headers = req.headers or {"Content-Type": "application/json"}
        
        if method == "POST":
            resp = requests.post(url, json=body, headers=headers, timeout=5)
        elif method == "GET":
            resp = requests.get(url, params=body, headers=headers, timeout=5)
        else:
            resp = requests.request(method, url, json=body, headers=headers, timeout=5)
        ...
        return {
            ...
            "body": {"error": str(e), "message": "Sandbox request failed or server is booting"}
        }
